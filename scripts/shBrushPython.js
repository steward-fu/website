...
;(function()
{
  ...
  function Brush(){
    var my_constant = 'SERVICE_WIN32 SERVICE_STATE_ALL win32con win32service iter hashlib GENERIC_READ SC_MANAGER_ALL_ACCESS subprocess gtk glib MySQLdb idaapi os False True sys commands argparse gridfs pymongo MongoClient win32com.client win32 easywebadv password username protocol port verify_ssl googletrans Translator psutil __future__ print_function httplib2 io googleapiclient discovery oauth2client client tools Storage MediaFileUpload MediaIoBaseDownload acknowledgeAbuse fileId pageSize fields PyApp Window'
    var my_keyword =  'ord with return import from def while if default for range break in else elif __init__ class super replace int split try except None'
    var my_type = 'path serial threading time stdout args self dict list'
    var my_function = 'OpenSCManager next Open Cells Close EnumServicesStatus ListServices Quit hexdigest sha1 decode_insn ph_get_regnames exists open ArgumentParser add_argument readchar input raw_input start parse_args close read write len sleep exit print geteuid Serial Thread StatusIcon set_from_stock set_from_file format connect get_current_event get_current_event_time getoutput MenuItem ImageMenuItem Menu append Image set_image set_subimage SeparatorMenuItem set_submenu show_all popup system main_quit main cursor execute fetchall commit authenticate GridFS put get_version logout Dispatch CreateItem Add Send ls upload walk translate Process getpit memory_info get authorize Http build files get_media FileIO next_chunk create set_default_size set_title Label add'
